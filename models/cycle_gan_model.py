...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--roi_size', type=int, default=384,
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionLuminance1 = networks.LuminanceLoss(self.opt.Luminance_size1, self.opt.Luminance_size1, self.opt.roi_size).to(self.device)
            self.criterionLuminance2 = networks.LuminanceLoss(self.opt.Luminance_size2, self.opt.Luminance_size2, self.opt.roi_size).to(self.device)
            self.criterionStructure = networks.StructureLoss(channel=opt.output_nc, window_size=self.opt.Structure_size, crop_size=self.opt.roi_size).to(self.device)
    ...
    def backward_G(self):
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_Luminance_B + self.loss_Structure_A + self.loss_Structure_B
